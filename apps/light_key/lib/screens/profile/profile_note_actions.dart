...
/// Profile画面でのノートアクション実装。
///
/// ProfileProviderと連携して、リアクション/リノート/リプライの投稿、
/// ノート詳細への遷移を行う。
/// ユーザータップは、表示中のプロフィールユーザーと異なる場合のみ遷移する。
class ProfileNoteActions with NoteActionsMixin implements NoteActions {
  ProfileNoteActions({
  ...
  Future<void> onUserTap(User user) async {
    // 現在表示中のプロフィールと同じユーザーの場合は何もしない
    if (user.id.isEmpty || user.id == currentUserId) {
      return;
    }
    // 異なるユーザーの場合はそのユーザーのプロフィールへ遷移
